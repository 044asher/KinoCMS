...
// Проверяем, есть ли элемент с сообщением об ошибке
    ...
    if (alertDanger) {
// Получаем текст сообщения из элемента
      var message = alertDanger.querySelector('p').textContent;
      Swal.fire({
        title: 'Ошибка!',
        text: message,
        customClass: {
...
// Скрываем элемент с сообщением об ошибке
...
// Показываем уведомление об ошибке, если в URL есть параметр 'error'
    if (urlParams.has('error')) {
      Swal.fire({
        icon: 'error',
        title: 'Ой...',
        text: 'Имя пользователя или пароль неверные!',
        customClass: {
          confirmButton: 'btn btn-primary'
        },
        buttonsStyling: false
...
// Если в URL есть параметр 'error', удаляем его перед отправкой формы
      if (urlParams.has('error')) {
        event.preventDefault();
        urlParams.delete('error');
        window.history.replaceState({}, '', window.location.pathname + (urlParams.toString() ? '?' + urlParams.toString() : ''));
        form.submit(); // Переотправляем форму после удаления параметра 'error'
      }
    });
  });
